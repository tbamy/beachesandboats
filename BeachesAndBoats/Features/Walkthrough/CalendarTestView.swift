...
class CalendarTestView: BaseViewControllerPlain {
    ...
    func setupCalendar() {
        ...
        calendarContent = calendarContent?.dayItemProvider { [weak self] day in
            guard let self = self else { return DayLabelTest.calendarItemModel(
            ...
            return DayLabelTest.calendarItemModel(
                invariantViewProperties: .init(
                    font: .systemFont(ofSize: 18),
...
struct DayLabelTest: CalendarItemViewRepresentable {
