...
//                print(response)
//                MiddleModal.show(title: "Success!", subtitle: response.message ?? "", type: .success, onConfirm: { self?.coordinator?.gotoListingSuccessView(type: 1) })
...
//            createBoatListing = savedListing
            ...
            // Convert saved image data back to UIImages
            if let savedImageData = savedListing.images {
    ...
    func setupCollectionView() {
        ...
        // Enable next button if we have saved images
//        nextBtn.isEnabled = !images.isEmpty
