...
extension RoomPriceView {
    func loadSavedData() {
        guard let savedListing = AppStorage.beachListing else { return }
        
        // If we're editing an existing room, load its price data
        if let roomIndex = room, roomIndex >= 0, let rooms = savedListing.rooms, roomIndex < rooms.count {
            let roomData = rooms[roomIndex]
            
            // Load stored values
            let storedPrice = roomData.pricePerNight ?? 0
            let storedDiscountPercent = Float(roomData.discountPercent ?? 0) / 100
