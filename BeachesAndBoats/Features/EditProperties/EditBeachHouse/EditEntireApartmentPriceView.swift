...
class EditEntireApartmentPriceView: UIViewController {
    ...
    func setup() {
        // Get stored values
        let storedPrice = details?.actual_price_per_night ?? 0
        let storedDiscountPercent = Float(details?.discountPercent ?? 0) / 100 // Additional discount percentage
    ...
    @IBAction func nextTapped(_ sender: Any) {
        ...
        if let beachData = beachData {
            ...
            if details?.bookingType?.isEmpty ?? true {
                createBeachListing?.bookingType = "FULL"
            }
            print("Request is: \(createBeachListing)")
            
            if let createBeachListing = createBeachListing {
                coordinator?.gotoEditEntireApartmentPricePerDayView(beachData: beachData, request: createBeachListing, details: details, id: id)
            }
