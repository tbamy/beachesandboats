...
class EditRoomPriceView: BaseViewControllerPlain {
    ...
    func setup(){
        ...
        moneyField.text = String(room.pricePerNight ?? 0)
        commissionField.text = String(format: "You earn ₦%.2f", finalEarnings)
        finalDiscountPercent = room.discountPercent ?? 0.1
    ...
    @IBAction func nextTapped(_ sender: Any) {
        ...
        coordinator?.gotoEditRoomPricePerDayView(beachData: beachData, request: updatedBeachListing, room: room)
    }
