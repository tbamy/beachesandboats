...
@IBDesignable
public class TimePicker: InputField {
    var picker = UIDatePicker()
    
    public var selectedTime: Date? {
        didSet {
            if let selectedTime {
                updateTextField(with: selectedTime)
            } else {
                textField.text = ""
            }
        }
    ...
    override func setup() {
        ...
        // Attach picker and toolbar to textField
        textField.inputView = picker
        textField.inputAccessoryView = toolBar
        
        // Handle tapping on textBlocker
        textBlocker.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(pickerTapped(_:))))
    }
